...
`CommentPanel.tsx` is the comment UI — a permanent side panel on desktop, wired into `commentStore`, with pagination, live-highlight for WS-delivered comments, skeleton/error/empty states, optimistic single-tap-to-like reactions. (An unused `CommentSheet.tsx` bottom-sheet variant existed alongside it for a while, never imported anywhere — removed in a later cleanup pass rather than left as dead code.)
