...
| `/welcome` | `guest` | Landing screen post-onboarding: "Hop in" / "Join Kampos" CTAs, `GistPreviewMarquee` on desktop. Real bug fixed here: the page used an uncapped `min-h-dvh` with a fixed-height preview card, so on a short phone viewport (iPhone SE-class) the CTA button(s) got pushed below the fold with no way to scroll to them — now `h-dvh` + `overflow-hidden`, with the preview card sized by flex so it's the part that shrinks on a short screen instead. |
...
- **Poster-picked hero color:** a palette button beside the compose sheet's char-count ring opens a swatch strip (only reachable while the colored hero treatment would actually render — short text, no media). The compose textarea itself previews it live once picked (background, centered text, avatar hidden, same shrink-to-fit sizing as the real posted card) — composing genuinely shows what posting will look like, not a blind guess. Creation-only for now; editing an existing gist's color isn't wired up (the edit `PATCH` route doesn't carry `color_key` through yet). Backend: nullable `gists.color_key`, whitelisted against `GIST_COLOR_KEYS` at both the Zod schema and controller layer (`KamposBackend/Kampos.md` Section 4) — falls back to the hash-based color when null, same as before this existed. A real "Maximum update depth exceeded" crash surfaced while building this preview: the box had `min-h-[160px]` with no `min-h-0`/fixed height, so a flex child's default `min-height: auto` let it grow to match the textarea's own JS-managed content height — which then fed back into the *next* fit calculation as a moving target, an infinite loop. Fixed with `min-h-0` + an explicit `minHeight` style, giving the fit routine a stable, content-independent box to measure against.
- **Reactions:** double-tap-anywhere (text-only cards only — media cards' tap gestures are already spoken for) triggers a LOVE reaction plus a big center-screen Lottie burst; the desktop reaction row (`ReactionButton`) offers all 5 types (LIKE/LOVE/FIRE/SAD/LAUGH) with per-type counts, optimistic local deltas, and a shared `onReacted` callback so both entry points (double-tap and row-click) drive the same celebratory burst. **Mobile swaps this whole row for `MobileReactionBadge.tsx`** — a wide 5-emoji row doesn't fit mobile's tighter footer next to the comment button. It renders a single "hero" face (whichever reaction is currently active, or a neutral icon if none) with the other four as small satellites orbiting it at fixed diagonal points; tapping the hero opens a vertical tray of all 5 with live counts. `ProfileGistCard` (Section 5.3a) reuses the same hero+orbit *look* but is its own separate implementation, not this component — it needs the picker to open centered over the card's body instead of rising as a tray, which this component doesn't support.
- **Footer, mobile-narrow fix:** the stats cluster (views/reactions/shares) is `flex-nowrap` with its own `overflow-x-auto` (not `flex-wrap`) specifically so it can never push the reaction badge onto its own line below it on a genuinely narrow phone (~320-340px) — verified against real breakpoints, since `flex-wrap` was silently doing exactly that.
...
- **Reactions — mobile:** same hero+orbit *look* as `GistCard`'s `MobileReactionBadge` (Section 5.3), but its own local implementation, not that component — tapping the hero opens all 5 reactions centered *over the card's own body* instead of a tray rising off the trigger, which wouldn't have room to rise on a short list card. Same "click outside this card closes it" handling the action menu already has — needed specifically because with many cards in one list, opening a different card's picker is a click entirely outside the first one's own DOM.
...
- **`Profile`**: now also carries `campus_name`/`major_name` (full joined names, not just the short `campus_tag`/`major_tag`) on a student profile fetch — see Section 4a's profile-page write-up and `KamposBackend/Kampos.md` Section 2.
...
- **Avitag** — a user's unique handle/username, chosen during the setup wizard's last step, and also that profile's own URL (`/avitag`, Section 4a) — which is why a handle can't collide with any of this app's own route names (Section 10).
- **Profile** — a public identity (student, creator, company, school, or admin) tied to an account; this frontend's UI only actually builds and publicly displays *student* profiles (`createStudentProfile`, `/[avitag]`) — the setup wizard has no path for the other profile types, and neither does the public profile page, even though the backend supports all five.
