...
import { useProfilesAdminStore, type KompanyProfileRow } from "@/stores/profilesAdminStore";
...
/**
 * Kompany tab of /villagepeople/profiles — search + status/verified filters
 * only, per the task's own "none beyond search+status+verified for
 * Kompany/Idiot" instruction (no campus concept for this type). Same
 ...
 *
 * Two DIFFERENT emails on this type: `email` (this profile's own separate
 * contact email, confirmed DB column `kompany_profiles.email`) vs the login
 * account's email (joined in — task spec names it `accountEmail`, read
 * defensively against `account_email` too since no admin search controller
 * exists yet to confirm the real key — see serverProfilesAdmin.ts's own doc
 * comment on KompanyProfileRow).
 *
 ...
 */
export function KompanyProfilesTab({
  initialRows,
  onFail,
  onSucceed,
}: {
  initialRows: KompanyProfileRow[];
  onFail: (msg: string) => void;
  onSucceed: (msg: string) => void;
}) {
  const router = useRouter();
  const [rows, setRows] = useState<KompanyProfileRow[]>(initialRows);
  ...
  const [deleteTarget, setDeleteTarget] = useState<KompanyProfileRow | null>(null);
  const [banTarget, setBanTarget] = useState<KompanyProfileRow | null>(null);
  ...
  const fetchKompanies = useProfilesAdminStore((s) => s.fetchKompanies);
  ...
  const runSearch = async (filters: ProfileAdminFilters) => {
    setSearching(true);
    try {
      const results = await fetchKompanies({ ...filters, limit: PAGE_SIZE });
      setRows(results);
      setExhausted(false);
    } catch (err) {
      onFail(apiErrorMessage(err, "Failed to load kompany profiles"));
    } finally {
      setSearching(false);
    }
  };
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const more = await fetchKompanies({ ...currentFilters(), cursor, limit: PAGE_SIZE });
      if (more.length) setRows((prev) => [...prev, ...more]);
      else setExhausted(true);
    } catch (err) {
      onFail(apiErrorMessage(err, "Failed to load more kompany profiles"));
    } finally {
      setLoadingMore(false);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loadingMore, searching, exhausted, rows, fetchKompanies, status, verified]);
  ...
  const handleFilterChange = (
    next: Partial<{ status: AdminProfileStatus | ""; verified: "" | "true" | "false" }>,
  ) => {
  ...
  const handleVerify = async (row: KompanyProfileRow) => {
    const nextVerified = !row.is_verified;
    setBusyAvitag(row.avitag);
    setBusyAction("verify");
    try {
      if (nextVerified) await verifyProfile("kompany", row.avitag);
      else await unverifyProfile("kompany", row.avitag);
  ...
  const handleDelete = async (reason?: string) => {
    ...
    try {
      await deleteProfile("kompany", deleteTarget.avitag, reason);
      setRows((prev) => prev.filter((r) => r.avitag !== deleteTarget.avitag));
      onSucceed(`@${deleteTarget.avitag}'s kompany profile was deleted.`);
  ...
  const handleToggleBan = async (row: KompanyProfileRow, reason?: string) => {
    const nextBanned = row.profile_status !== "BANNED";
    setBusyAvitag(row.avitag);
    setBusyAction("ban");
    try {
      if (nextBanned) await banProfile("kompany", row.avitag, reason);
      else await unbanProfile("kompany", row.avitag);
      setRows((prev) =>
  ...
  const fieldsFor = (row: KompanyProfileRow) =>
    buildFields([
      ["Contact email", row.email],
      ["Login email", row.accountEmail ?? row.account_email],
  ...
  return (
    <div className="flex flex-col gap-4">
      <ConfirmReasonModal
        ...
        title="Delete this kompany profile?"
      ...
      <ConfirmReasonModal
        ...
        title="Ban this kompany profile?"
      ...
      {searching ? (
        ...
        <p className="rounded-2xl border border-dashed border-line/70 p-6 text-center font-nunito text-sm text-muted">
          No kompany profiles match this search.
        </p>
      ) : (
        <>
          <ul className="flex flex-col gap-3">
            {rows.map((row) => (
              <ProfileRowCard
                ...
                onView={() => router.push(`/villagepeople/profiles/${PROFILE_TYPE_PATH.kompany}/${row.avitag}`)}
                onEdit={() => router.push(`/villagepeople/profiles/${PROFILE_TYPE_PATH.kompany}/${row.avitag}/edit`)}
          ...
          {exhausted && (
            <p className="py-3 text-center font-nunito text-xs text-faint">
              That&apos;s every kompany profile matching this search.
            </p>
          )}
        </>
      )}
