...
import { useProfilesAdminStore, type IdiotProfileRow } from "@/stores/profilesAdminStore";
...
/**
 * Idiot tab of /villagepeople/profiles — search + status/verified filters
 * only, per the task's own "none beyond search+status+verified for
 * Kompany/Idiot" instruction. Same cursor-based infinite scroll as
 * StudentProfilesTab — see that file's own doc comment for the full "why
 * cursor, not offset" reasoning.
 *
 * "Idiot" here is the PROFILE type (a moderation-facing profile) — distinct
 * from the account-level 'idiot'/'king' admin role of the same name that
 * gates this whole /villagepeople section. See profileEditFields.ts's own
 * note on this exact naming collision.
 *
 ...
 */
export function IdiotProfilesTab({
  initialRows,
  onFail,
  onSucceed,
}: {
  initialRows: IdiotProfileRow[];
  onFail: (msg: string) => void;
  onSucceed: (msg: string) => void;
}) {
  const router = useRouter();
  const [rows, setRows] = useState<IdiotProfileRow[]>(initialRows);
  ...
  const [deleteTarget, setDeleteTarget] = useState<IdiotProfileRow | null>(null);
  const [banTarget, setBanTarget] = useState<IdiotProfileRow | null>(null);
  ...
  const fetchIdiots = useProfilesAdminStore((s) => s.fetchIdiots);
  ...
  const runSearch = async (filters: ProfileAdminFilters) => {
    setSearching(true);
    try {
      const results = await fetchIdiots({ ...filters, limit: PAGE_SIZE });
      setRows(results);
      setExhausted(false);
    } catch (err) {
      onFail(apiErrorMessage(err, "Failed to load idiot profiles"));
    } finally {
      setSearching(false);
    }
  };
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const more = await fetchIdiots({ ...currentFilters(), cursor, limit: PAGE_SIZE });
      if (more.length) setRows((prev) => [...prev, ...more]);
      else setExhausted(true);
    } catch (err) {
      onFail(apiErrorMessage(err, "Failed to load more idiot profiles"));
    } finally {
      setLoadingMore(false);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loadingMore, searching, exhausted, rows, fetchIdiots, status, verified]);
  ...
  const handleVerify = async (row: IdiotProfileRow) => {
    const nextVerified = !row.is_verified;
    setBusyAvitag(row.avitag);
    setBusyAction("verify");
    try {
      if (nextVerified) await verifyProfile("idiot", row.avitag);
      else await unverifyProfile("idiot", row.avitag);
  ...
  const handleDelete = async (reason?: string) => {
    ...
    try {
      await deleteProfile("idiot", deleteTarget.avitag, reason);
      setRows((prev) => prev.filter((r) => r.avitag !== deleteTarget.avitag));
      onSucceed(`@${deleteTarget.avitag}'s idiot profile was deleted.`);
  ...
  const handleToggleBan = async (row: IdiotProfileRow, reason?: string) => {
    const nextBanned = row.profile_status !== "BANNED";
    setBusyAvitag(row.avitag);
    setBusyAction("ban");
    try {
      if (nextBanned) await banProfile("idiot", row.avitag, reason);
      else await unbanProfile("idiot", row.avitag);
      setRows((prev) =>
  ...
  const fieldsFor = (row: IdiotProfileRow) =>
    buildFields([
      ["Email", row.email],
  ...
  return (
    <div className="flex flex-col gap-4">
      <ConfirmReasonModal
        ...
        title="Delete this idiot profile?"
      ...
      {searching ? (
        ...
        <p className="rounded-2xl border border-dashed border-line/70 p-6 text-center font-nunito text-sm text-muted">
          No idiot profiles match this search.
        </p>
      ) : (
        <>
          <ul className="flex flex-col gap-3">
            {rows.map((row) => (
              <ProfileRowCard
                ...
                onView={() => router.push(`/villagepeople/profiles/${PROFILE_TYPE_PATH.idiot}/${row.avitag}`)}
                onEdit={() => router.push(`/villagepeople/profiles/${PROFILE_TYPE_PATH.idiot}/${row.avitag}/edit`)}
          ...
          {exhausted && (
            <p className="py-3 text-center font-nunito text-xs text-faint">
              That&apos;s every idiot profile matching this search.
            </p>
          )}
        </>
      )}
