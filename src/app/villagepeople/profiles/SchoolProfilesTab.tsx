...
import { useProfilesAdminStore, type SchoolProfileRow } from "@/stores/profilesAdminStore";
...
/**
 * School tab of /villagepeople/profiles — search + status/verified filters
 * plus a campus dropdown. Same cursor-based infinite scroll as
 ...
 */
export function SchoolProfilesTab({
  initialRows,
  onFail,
  onSucceed,
}: {
  initialRows: SchoolProfileRow[];
  onFail: (msg: string) => void;
  onSucceed: (msg: string) => void;
}) {
  const router = useRouter();
  const [rows, setRows] = useState<SchoolProfileRow[]>(initialRows);
  ...
  const [deleteTarget, setDeleteTarget] = useState<SchoolProfileRow | null>(null);
  const [banTarget, setBanTarget] = useState<SchoolProfileRow | null>(null);
  ...
  const fetchSchools = useProfilesAdminStore((s) => s.fetchSchools);
  ...
  const runSearch = async (filters: ProfileAdminFilters) => {
    setSearching(true);
    try {
      const results = await fetchSchools({ ...filters, limit: PAGE_SIZE });
      setRows(results);
      setExhausted(false);
    } catch (err) {
      onFail(apiErrorMessage(err, "Failed to load school profiles"));
    } finally {
      setSearching(false);
    }
  };
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const more = await fetchSchools({ ...currentFilters(), cursor, limit: PAGE_SIZE });
      if (more.length) setRows((prev) => [...prev, ...more]);
      else setExhausted(true);
    } catch (err) {
      onFail(apiErrorMessage(err, "Failed to load more school profiles"));
    } finally {
      setLoadingMore(false);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loadingMore, searching, exhausted, rows, fetchSchools, status, verified, campusTag]);
  ...
  const handleVerify = async (row: SchoolProfileRow) => {
    const nextVerified = !row.is_verified;
    setBusyAvitag(row.avitag);
    setBusyAction("verify");
    try {
      if (nextVerified) await verifyProfile("school", row.avitag);
      else await unverifyProfile("school", row.avitag);
  ...
  const handleDelete = async (reason?: string) => {
    ...
    try {
      await deleteProfile("school", deleteTarget.avitag, reason);
      setRows((prev) => prev.filter((r) => r.avitag !== deleteTarget.avitag));
      onSucceed(`@${deleteTarget.avitag}'s school profile was deleted.`);
  ...
  const handleToggleBan = async (row: SchoolProfileRow, reason?: string) => {
    const nextBanned = row.profile_status !== "BANNED";
    setBusyAvitag(row.avitag);
    setBusyAction("ban");
    try {
      if (nextBanned) await banProfile("school", row.avitag, reason);
      else await unbanProfile("school", row.avitag);
      setRows((prev) =>
  ...
  const fieldsFor = (row: SchoolProfileRow) =>
    buildFields([
      ["Email", row.email],
      ["Campus", row.campus_tag],
      ["Website", row.website],
  ...
  return (
    <div className="flex flex-col gap-4">
      <ConfirmReasonModal
        ...
        title="Delete this school profile?"
      ...
      {searching ? (
        ...
        <p className="rounded-2xl border border-dashed border-line/70 p-6 text-center font-nunito text-sm text-muted">
          No school profiles match this search.
        </p>
      ) : (
        <>
          <ul className="flex flex-col gap-3">
            {rows.map((row) => (
              <ProfileRowCard
                ...
                onView={() => router.push(`/villagepeople/profiles/${PROFILE_TYPE_PATH.school}/${row.avitag}`)}
                onEdit={() => router.push(`/villagepeople/profiles/${PROFILE_TYPE_PATH.school}/${row.avitag}/edit`)}
          ...
          {exhausted && (
            <p className="py-3 text-center font-nunito text-xs text-faint">
              That&apos;s every school profile matching this search.
            </p>
          )}
        </>
      )}
